...
每日自动抓取国内外主流新闻源，聚合后国内/国际各取 TOP 10，通过 Server酱 / PushPlus 推送到微信。推送内容包含标题、中文概括、相关图片和来源链接，不再只发送链接列表。配置 PushPlus 时会优先使用 PushPlus 的 HTML 模板发送图文，Server酱作为备用。
...
为提高微信端图片显示率，PushPlus 使用 `html` 模板和 `<img>` 标签发送图文；如果图片仍被微信拦截，消息会保留原图链接。

推送前会输出质量报告，包括候选数、入选来源分布、丢弃原因、图片数量、Google News 聚合链接残留和乱码残留。最终 TOP 10 不再只按热度排序，而是综合来源权威度、时效性、正文质量、图片可用性和链接质量评分。
